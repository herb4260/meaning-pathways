...
mod_vars <- c(
  "mv_t1", "prc_t2", "struggle_t2", "meaning_t3",
  "recovery_t4", "recovery_t1", "baseline_stressor_severity"
)
mod_dat <- dat[, mod_vars, drop = FALSE]
mod_dat[] <- lapply(mod_dat, as.numeric)
stopifnot(!anyDuplicated(names(mod_dat)))

for (v in mod_vars) {
  mod_dat[[paste0("z_", v)]] <- as.numeric(scale(mod_dat[[v]]))
}
mod_dat$prcXstruggle_t2 <- mod_dat$z_prc_t2 * mod_dat$z_struggle_t2
